# This Makefile is for the File::Path extension to perl.
#
...
#     ABSTRACT_FROM => q[Path.pm]
#     AUTHOR => q[David Landgren]
#     LICENSE => q[perl]
#     NAME => q[File::Path]
#     PREREQ_PM => { File::Basename=>q[0] }
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  Path.pm $(INST_MAN3DIR)/File::Path.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                File-Path' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             2.00_06' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Create or remove directory trees' >> META_new.yml
	$(NOECHO) $(ECHO) 'license:             perl' >> META_new.yml
	$(NOECHO) $(ECHO) 'generated_by:        ExtUtils::MakeMaker version 6.32' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    - David Landgren' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="2,00_06,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Create or remove directory trees</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>David Landgren</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Basename" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <OS NAME="$(OSNAME)" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="i386-freebsd-64int-5.8" />' >> $(DISTNAME).ppd
